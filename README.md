...
Note that the UMC1820 [absolutely does NOT require attenuators to handle modular signals directly](https://modwiggler.com/forum/viewtopic.php?p=2936306#p2936306), indeed, it is even specified to be capable of handling inputs up to +18dBu. However, this requires that the inputs are set to LINE (obviously), that the PADs are applied, and the GAIN controls are set way down near the bottom of their travel. I have been pleased to discover that in addition to service as front end of a DAW, the UMC1820 also operates usefuly as a stand-alone mixer, with 8 channels going in and direct output to the system monitors. In this application, some attenuation on the inputs is helpful, as it allows the GAIN controls to operate as Level controls over a wider control range, with more useful resolution.
